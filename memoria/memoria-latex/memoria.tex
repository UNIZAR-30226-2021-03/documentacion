...
\textbf{Pi.1} Identificar y asignar recursos.

\textbf{Pi.2} Servidor \textit{cloud} para el despliegue.

\textbf{Pi.3} Base de datos a utilizar.

\textbf{Pi.4} Formación y documentación inicial.

\subsubsection{Procesos de ejecución y control}

\textbf{Pi.1} Comunicación y concreción de los horarios de trabajo via \textit{Whatsapp}.

\textbf{Pi.2} Reunión semanal de puesta en común via \textit{Google Meet} en la cual estarán presentes todos los miembros del grupo.

\textbf{Pi.3} Registro de las decisiones tomadas en las reuniones semanales en actas almacenadas en \textit{GitHub}.

\textbf{Pi.4} Semanalmente monitorización del avance de módulos, determinación de tareas a realizar, asignación de tareas a desarrolladores y marcaje de límites temporales y prioridades de cada tarea.

\textbf{Pi.5} Registrar código en las \textit{Wikis} de \textit{GitHub}.

\textbf{Pi.6} Desarrolladores rellenarán una tabla de control de esfuerzos para registrar su trabajo.

\textbf{Pi.7} La entrega de resultados se realizará de manera continua.
...
\begin{table}[H]
    ...
    \hline
        ...
        RF-4.3 & El sistema muestra el grado de robustez (entropía) de la contraseña al ser generada. La entropía($E$) se calcula: $E=n\log_2m$. Donde $n$ es la longitud y $m$ el número de carácteres de la población usada.\\ \hline
        ...
        RF-9 & Se accede al sistema mediante una aplicación para dispositivos móviles Android con versión 6.1 (\textit{Marshmallow}). \\ \hline
        RF-10 & Se accede al sistema mediante una aplicación para navegadores web Google Chrome (versión 89) y Firefox (versión 87). \\ \hline
        RF-11 & El sistema ofrece un \textit{plug-in} para el navegador Google Chrome (versión 89). \\ \hline
    \end{tabular}
\end{table}
